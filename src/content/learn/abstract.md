...
The abstract is the short summary that sits between your title and your first section. Nearly every venue requires one. LaTeX gives it a dedicated environment rather than leaving you to fake it with a bold heading. The class needs to know which text is the abstract so journal classes can typeset it in a distinct style (often smaller and indented), and so submission systems can extract it automatically.
...
Everything between `\begin{abstract}` and `\end{abstract}` becomes the abstract block. In the standard `article` class it appears as a narrowed, centered-heading paragraph under the title. Write plain prose inside it. Inline math is fine, but skip anything heavyweight: no citations if the venue forbids them, no footnotes, no undefined acronyms. Abstracts often show up alone in databases and search results, away from the rest of the paper.
