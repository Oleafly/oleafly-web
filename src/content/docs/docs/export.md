...
Different from exporting: the [PDF preview's](/docs/pdf-preview/#the-toolbar-left-to-right) **Save PDF to project** writes the compiled PDF into the project tree itself, committed via Git. Use it to snapshot "the version I submitted" next to your source.

## Tagged, accessible PDFs

For a Section 508 / PDF-UA oriented PDF with a real tag structure, use the [Preflight accessible-export flow](/docs/preflight/#accessible-tagged-pdf-export); the result lands in the same build output and downloads like any other PDF.
