...
2. **You don't.** One button, **Install TinyTeX (~100 MB)**, downloads a self-contained TeX Live into `~/.oleafly/tinytex` with live progress. No admin rights, no system changes, and **Delete TinyTeX to free space** removes it entirely whenever you like.
...
Templates declare the packages they need (IEEEtran, acmart, elsarticle, beamer, tikzposter, natbib, and friends), and Tectonic fetches them on first compile. Templates with on-demand fonts handle those separately; see [Templates](/docs/templates/#on-demand-fonts-the-setup-badge).
