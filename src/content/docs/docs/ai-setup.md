...
Each provider card carries a **Get key** link straight to the right page. Reasoning models (GLM, DeepSeek R1) stream their thinking phase live into the chat; see [Chat & tools](/docs/ai-chat/#reasoning-models-think-out-loud).
...
![Provider cards in Settings, AI Assistant](/media/settings-ai.png)
...
The settings page lists the assistant's tools with plain-English descriptions, from `read_file` to `compile`. The full story, including the approval flow that gates every file change, is on [Chat & tools](/docs/ai-chat/).
