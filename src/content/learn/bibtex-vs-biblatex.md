...
The style row matters most. BibTeX styles are `.bst` files that almost nobody can read or modify, so you take what the journal ships. biblatex styles are ordinary LaTeX, split into bibliography (`.bbx`) and citation (`.cbx`) components, and small changes like dropping URLs or altering punctuation are one-line customizations. The Unicode row matters if your references contain accented names or non-Latin scripts. Classic BibTeX mangles them unless every character is escaped as `\'{e}` and friends, while biber handles UTF-8 natively. The last row is why classic BibTeX survives: most journals and conferences still distribute a `.bst` file and expect the classic toolchain, and submission systems often refuse anything else.
