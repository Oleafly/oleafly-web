...
- **Delete**: removes the file after a confirmation. This cannot be undone from the app, though [Git history](/docs/git-history/) can bring back anything you had committed.
...
Open files appear as tabs above the editor, in the order you opened them. A dot on the tab means unsaved changes are pending (they'll autosave in a moment). Git [diff views](/docs/git-history/#the-diff-viewer) open as tabs in the same strip, labeled with the file name plus `(Index)` or `(Working Tree)`.
...
- **`.bib` files** are also parsed in the background the moment a project opens, which is what powers [citation autocomplete](/docs/citations/#autocomplete-from-your-bib).
