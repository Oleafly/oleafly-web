...
A control sequence is anything that starts with a backslash. TeX has no fixed vocabulary: commands exist only because the kernel, the document class, a package, or your own `\newcommand` defined them. When TeX reads a backslash followed by a name that nothing has defined, it cannot guess what you meant and stops. The error is never about the command being "wrong" in general. It is about that name being unknown in this document, at this point, under this engine.
